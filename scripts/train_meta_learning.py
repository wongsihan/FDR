...
# -*- coding: utf-8 -*-
"""
Meta-learning training script for Few-shot multiscene fault diagnosis
"""
...
from src.data.data_generator import pu_folders, pu_folders_anchor, set_seed, PUTask
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Meta-learning for Few-shot multiscene fault diagnosis")
    ...
    parser.add_argument("--test_gap", type=int, default=5, help="Test every N episodes")
    ...
    parser.add_argument("--result_root", type=str, default="./results", help="Result root directory")
...
def load_data(config):
    ...
    if not os.path.exists(datapath):
        ...
        logging.info("Loading cached training data...")
    ...
    if not os.path.exists(datapath2):
        ...
        logging.info("Loading cached anchor data...")
        with open(datapath2, 'rb') as f:
            metatrain_character_folders_anchor, metatest_character_folders_anchor = pickle.load(f)
    
    return (metatrain_character_folders, metatest_character_folders, 
            metatrain_character_folders_anchor, metatest_character_folders_anchor)
...
def create_models(config):
    ...
    if config.modeltype == '1d':
        ...
        feature_encoder_anchor = CNNEncoder2d(config.feature_dim)  # Note: 2D version doesn't have anchor
    ...
    return feature_encoder, feature_encoder_anchor, relation_network


def create_optimizers(feature_encoder, feature_encoder_anchor, relation_network, config):
    ...
    return ((feature_encoder_optim, feature_encoder_scheduler),
            (feature_encoder_optim_anchor, feature_encoder_scheduler_anchor),
            (relation_network_optim, relation_network_scheduler))


def train_episode(feature_encoder, feature_encoder_anchor, relation_network, 
                 optimizers, config, metatrain_character_folders, 
                 metatrain_character_folders_anchor, metatest_character_folders_anchor):
    """Train one episode"""
    (feature_encoder_optim, feature_encoder_scheduler), \
    (feature_encoder_optim_anchor, feature_encoder_scheduler_anchor), \
    (relation_network_optim, relation_network_scheduler) = optimizers
...
def test_model(feature_encoder, relation_network, config, metatest_character_folders):
    """Test the model"""
...
def main():
    ...
    logging.info("Starting meta-learning training...")
    logging.info(f"Configuration: {config}")
    
    # Load data
    data = load_data(config)
    metatrain_character_folders, metatest_character_folders, metatrain_character_folders_anchor, metatest_character_folders_anchor = data
    
    # Create models
    feature_encoder, feature_encoder_anchor, relation_network = create_models(config)
    
    # Create optimizers
    optimizers = create_optimizers(feature_encoder, feature_encoder_anchor, relation_network, config)
    ...
    # Progress bar
    pbar = tqdm(range(config.episode), desc="Training")
    
    for episode in pbar:
        # Update learning rates
        for scheduler in [optimizers[0][1], optimizers[1][1], optimizers[2][1]]:
            scheduler.step(episode)
        
        # Train one episode
        loss = train_episode(feature_encoder, feature_encoder_anchor, relation_network, 
                           optimizers, config, metatrain_character_folders, 
                           metatrain_character_folders_anchor, metatest_character_folders_anchor)
        ...
        if (episode + 1) % args.test_gap == 0:
            logging.info("Testing...")
            test_accuracy = test_model(feature_encoder, relation_network, config, metatest_character_folders)
            
            logging.info(f"Episode {episode + 1}: Test accuracy = {test_accuracy:.4f}")
