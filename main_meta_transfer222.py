...
from utlis.models2 import CNNEncoder1d, CNNEncoder2d, RelationNetwork1d, RelationNetwork2d
...
parser.add_argument("-ca", "--causalityration", type=int, default=0.01)
...
def main():
    ...
    for episode in range(EPISODE):
        ...
        sample_features_o_anchor = feature_encoder(Variable(samples).cuda(GPU).float())[1]
        sample_features_o_positive = feature_encoder(Variable(samples).cuda(GPU).float())[2]
        sample_features_o_negative = feature_encoder(Variable(samples).cuda(GPU).float())[3]
        if MODELTYPE == '1d':
    ...
    #loss caculate
        ...
        loss_causality = CAU * get_causality_loss(get_entropy(sample_features_o_anchor),
        ...
        one_hot_labels = Variable(
            torch.zeros(BATCH_NUM_PER_CLASS * CLASS_NUM, CLASS_NUM).scatter_(1, batch_labels.cuda().long().view(-1, 1),
                                                                             1)).cuda(GPU)
        loss = mse(relations, one_hot_labels) + loss_causality
