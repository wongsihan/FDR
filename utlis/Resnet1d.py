...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        identity = x
        #print("x is: ",identity.size())
        out = self.conv1(x)
        out = self.bn1(out)
        out = self.relu(out)
        #print("conv1 is: ", out.size())
        out = self.conv2(out)
        out = self.bn2(out)
        #print("conv2 is: ", out.size())
...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        #print("origin x is:",x.size())
        x = self.conv1(x)
        #print("origin conv1 x is:", x.size())
        x = self.bn1(x)
        #print("origin bn1 x is:", x.size())
        x = self.relu(x)
        #print("origin relu1 x is:", x.size())
        x = self.maxpool(x)
        #print("origin maxpool is:", x.size())

        x = self.layer1(x)
        #print("first stage:",x.size())
        x = self.layer2(x)
        x = self.layer3(x)
        x = self.layer4(x)
