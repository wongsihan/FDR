...
from utlis.models7 import CNNEncoder1d, CNNEncoder2d, RelationNetwork1d, RelationNetwork2d
...
def main():
    ...
    for episode in range(EPISODE):
        ...
        conv_normal1 = feature_encoder(Variable(samples).cuda(GPU).float())[1][0]
        conv_noise1 = feature_encoder(Variable(samples2).cuda(GPU).float())[1][0]
        logp_x = F.log_softmax(conv_normal1, dim=-1)
        p_y = F.softmax(conv_noise1, dim=-1)

        kl_sum = F.kl_div(logp_x, p_y, reduction='sum')
        kl_mean = F.kl_div(logp_x, p_y, reduction='mean')
        # print("kl1 sum is: ",kl_sum)
        print("kl1 mean is: ", kl_mean)

        conv_normal2 = feature_encoder(Variable(samples).cuda(GPU).float())[1][1]
        conv_noise2 = feature_encoder(Variable(samples2).cuda(GPU).float())[1][1]
        logp_x = F.log_softmax(conv_normal2, dim=-1)
        p_y = F.softmax(conv_noise2, dim=-1)

        kl_sum2 = F.kl_div(logp_x, p_y, reduction='sum')
        kl_mean2 = F.kl_div(logp_x, p_y, reduction='mean')
        # print("kl2 sum is: ", kl_sum2)
        print("kl2 mean is: ", kl_mean2)

        conv_normal3 = feature_encoder(Variable(samples).cuda(GPU).float())[1][2]
        conv_noise3 = feature_encoder(Variable(samples2).cuda(GPU).float())[1][2]
        logp_x = F.log_softmax(conv_normal3, dim=-1)
        p_y = F.softmax(conv_noise3, dim=-1)

        kl_sum3 = F.kl_div(logp_x, p_y, reduction='sum')
        kl_mean3 = F.kl_div(logp_x, p_y, reduction='mean')
        # print("kl3 sum is: ", kl_sum3)
        print("kl3 mean is: ", kl_mean3)

        conv_normal4 = feature_encoder(Variable(samples).cuda(GPU).float())[1][3]
        conv_noise4 = feature_encoder(Variable(samples2).cuda(GPU).float())[1][3]
        logp_x = F.log_softmax(conv_normal4, dim=-1)
        p_y = F.softmax(conv_noise4, dim=-1)
        kl_mean4 = F.kl_div(logp_x, p_y, reduction='mean')
        print("kl4 mean is: ", kl_mean4)

        # conv_normal5 = feature_encoder(Variable(samples).cuda(GPU).float())[1][4]
        # conv_noise5 = feature_encoder(Variable(samples2).cuda(GPU).float())[1][4]
        # logp_x = F.log_softmax(conv_normal5, dim=-1)
        # p_y = F.softmax(conv_noise5, dim=-1)
        # kl_mean5 = F.kl_div(logp_x, p_y, reduction='mean')
        # print("kl5 mean is: ", kl_mean5)
        #
        # conv_normal6 = feature_encoder(Variable(samples).cuda(GPU).float())[1][5]
        # conv_noise6 = feature_encoder(Variable(samples2).cuda(GPU).float())[1][5]
        # logp_x = F.log_softmax(conv_normal6, dim=-1)
        # p_y = F.softmax(conv_noise6, dim=-1)
        # kl_mean6 = F.kl_div(logp_x, p_y, reduction='mean')
        # print("kl6 mean is: ", kl_mean6)
        #
        # conv_normal7 = feature_encoder(Variable(samples).cuda(GPU).float())[1][6]
        # conv_noise7 = feature_encoder(Variable(samples2).cuda(GPU).float())[1][6]
        # logp_x = F.log_softmax(conv_normal7, dim=-1)
        # p_y = F.softmax(conv_noise7, dim=-1)
        # kl_mean7 = F.kl_div(logp_x, p_y, reduction='mean')
        # print("kl7 mean is: ", kl_mean7)
        #
        # conv_normal8 = feature_encoder(Variable(samples).cuda(GPU).float())[1][7]
        # conv_noise8 = feature_encoder(Variable(samples2).cuda(GPU).float())[1][7]
        # logp_x = F.log_softmax(conv_normal8, dim=-1)
        # p_y = F.softmax(conv_noise8, dim=-1)
        # kl_mean8 = F.kl_div(logp_x, p_y, reduction='mean')
        # print("kl8 mean is: ", kl_mean8)
        #
        # conv_normal9 = feature_encoder(Variable(samples).cuda(GPU).float())[1][8]
        # conv_noise9 = feature_encoder(Variable(samples2).cuda(GPU).float())[1][8]
        # logp_x = F.log_softmax(conv_normal9, dim=-1)
        # p_y = F.softmax(conv_noise9, dim=-1)
        # kl_mean9 = F.kl_div(logp_x, p_y, reduction='mean')
        # print("kl9 mean is: ", kl_mean9)
        #
        # conv_normal10 = feature_encoder(Variable(samples).cuda(GPU).float())[1][9]
        # conv_noise10 = feature_encoder(Variable(samples2).cuda(GPU).float())[1][9]
        # logp_x = F.log_softmax(conv_normal10, dim=-1)
        # p_y = F.softmax(conv_noise10, dim=-1)
        # kl_mean10 = F.kl_div(logp_x, p_y, reduction='mean')
        # print("kl10 mean is: ", kl_mean10)
        ...
        if (episode + 1) % testgap == 0:
            ...
            for i in range(TEST_EPISODE):
                ...
                sample_dataloader = tg.get_data_loader(task, num_per_class=SAMPLE_NUM_PER_CLASS, split="train",
                                                       shuffle=False, dt=DATATYPE, mt=MODELTYPE,snr = SNR)
